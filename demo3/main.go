...
func SaveToDB(title string, content string) {
	...
	page := GormPage{Title: title, Content: content}
	affected, err := engine.Insert(&page)
	if err != nil {
		fmt.Printf("insert failed, err:%v\n", err)
		return
	}
	fmt.Println("save success!" + string(affected))
}

func init() {
	engine, err = xorm.NewEngine("mysql", "root:@/spider?charset=utf8")
	if err != nil {
		fmt.Printf("mysql login failed, err:%v\n", err)
		return
	} else {
		err2 := engine.Ping()
		if err2 != nil {
